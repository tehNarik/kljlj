...
// Позначка для перевірки, чи вже відображалося повідомлення
...
function readCookiesOnLoad() {
    ...
    if (maxDigitCookie && !messageDisplayed) {
        ...
        var userConfirmation = confirm('Інформація, збережена в cookies:\n\nМаксимальна цифра: ' + maxDigitValue +
            '\n\nНатисніть "ОК", щоб видалити cookies та перезавантажити сторінку. тест');
        if (userConfirmation) {
            deleteAllCookiesAndReload();
        }
    }
}

function deleteAllCookiesAndReload() {
    document.cookie = 'maxDigit=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;';

    // Позначити, що повідомлення вже відображалося
    messageDisplayed = true;

    // Перезавантаження сторінки після видалення кукі
    setTimeout(function () {
        // Зняти позначку, щоб повідомлення відображалося знову при наступному оновленні
